...
# clone vm and modify nics
data "cloudtower_vlan" "vlan233" {
  name       = "vlan233"
  type       = "VM"
  cluster_id = data.cloudtower_cluster.cluster.clusters[0].id
}

resource "cloudtower_vm" "foo-clone-2-modify-nics" {
  name       = "foo-clone-2-modify-nics"
  ...
  nic {
    vlan_id = data.cloudtower_vlan.vlan233.vlans[0].id
    enabled = true
  }
}
