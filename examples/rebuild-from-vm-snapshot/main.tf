...
resource "cloudtower_vm_snapshot" "foo-snapshot" {
  name            = "foo-snapshot-1"
  vm_id           = cloudtower_vm.foo.id
  consistent_type = "CRASH_CONSISTENT" # `CRASH_CONSISTENT` or `FILE_SYSTEM_CONSISTENT`
}

resource "cloudtower_vm" "foo-snapshot-1-vm-rebuild" {
  name       = "foo-snapshot-1-vm-rebuild"
  cluster_id = data.cloudtower_cluster.cluster.clusters[0].id
  create_effect {
    rebuild_from_snapshot = cloudtower_vm_snapshot.foo-snapshot.id
  }
}